...
class Request:
    ...
    def __init__(self, scope: Scope, receive: Receive):
        ...
        self.query_string = scope.get("query_string").decode(self.charset)
        self.query: typing.Dict[str, str] = {}
        if self.query_string:
            self.query = dict(urllib.parse.parse_qsl(self.query_string, encoding=self.charset))
    ...
    async def body(self):
        if not self._body:
            async for part in self._stream():
                self._body.update(dict(urllib.parse.parse_qsl(part.decode(self.charset), encoding=self.charset)))
        return self._body
